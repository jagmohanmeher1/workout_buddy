...
import { User, WorkoutSession, WorkoutType } from './types';
...
const INITIAL_SESSIONS: WorkoutSession[] = [
  {
    id: '1',
    host: { id: '2', name: 'Sarah Chen', avatar: 'https://picsum.photos/seed/sarah/200', bio: '', interests: [] },
    type: 'Yoga',
  ...
  {
    id: '2',
    host: { id: '3', name: 'Mike Ross', avatar: 'https://picsum.photos/seed/mike/200', bio: '', interests: [] },
    type: 'Gym',
...
const App: React.FC = () => {
  ...
  const renderView = () => {
    switch (view) {
      case 'home':
        return <Dashboard sessions={sessions} onJoin={handleJoinSession} onCreateClick={() => setView('create')} />;
      case 'create':
        return <CreateWorkout onCreate={handleCreateSession} onCancel={() => setView('home')} userLocation={userLocation} />;
      case 'buddy':
        return <FindBuddy userLocation={userLocation} />;
      case 'map':
    ...
    <div className="min-h-screen pb-20 md:pb-0">
      ...
      <nav className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 md:hidden flex justify-around py-3 z-50">
        ...
        <button onClick={() => setView('create')} className="flex flex-col items-center -mt-6">
          ...
          <span className="text-xs mt-1 font-medium text-gray-500">Add</span>
        </button>
        <button onClick={() => setView('buddy')} className={`flex flex-col items-center ${view === 'buddy' ? 'text-indigo-600' : 'text-gray-500'}`}>
          ...
          <span className="text-xs mt-1 font-medium">Buddies</span>
        </button>
